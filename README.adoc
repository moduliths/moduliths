...
* https://github.com/janScheible/pocketsaw[Pocketsaw] -- Compile time sub-module system, aimed at package group dependency organization within a Maven project. Where the sub-modules are directly defined in the codebase by classes annotated with `@SubModule`.
