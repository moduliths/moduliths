...
import de.olivergierke.moduliths.model.NamedInterface.TypeBasedNamedInterface;
	...
	private static List<TypeBasedNamedInterface> ofAnnotatedTypes(JavaPackage basePackage) {
		...
		basePackage.stream() //
				...
				.forEach(it -> {
					...
					if (!it.isAnnotatedWith(de.olivergierke.moduliths.NamedInterface.class)) {
						return;
					}

					de.olivergierke.moduliths.NamedInterface annotation = it
							.getAnnotationOfType(de.olivergierke.moduliths.NamedInterface.class);
